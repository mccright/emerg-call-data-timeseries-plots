...
def load_time_in_service_times(csv_path):
    records = []
    with open(csv_path, newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            row['time_in_service_in_seconds'] = int(row['time_in_service_in_seconds'])
            records.append(row)
    return records
...
def plot_scores_interactive_dropdown(records, output_port: int, output_path="scores.svg"):
    ...
    for team, recs in teams.items():
        ...
        time_in_service_in_seconds = [r['time_in_service_in_seconds'] for r in recs]

        fig.add_trace(go.Scatter(
            x=dates,
            y=time_in_service_in_seconds,
            mode="lines+markers",
            name=team,
    ...
    fig.update_layout(
        title="Team time_in_service_in_seconds Over Time",
        xaxis_title="incident_date",
        # yaxis_title="response_time_in_seconds",
        yaxis=dict(
            title='time_in_service_in_seconds (log scale)',
...
if __name__ == '__main__':
    ...
    records = load_time_in_service_times(csv_path)
    #
